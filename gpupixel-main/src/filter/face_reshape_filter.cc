...
const std::string kGPUPixelThinFaceFragmentShaderString = R"(
 ...
 uniform int hasFace;
 uniform float facePoints[111 * 2];

 uniform highp float aspectRatio;
 uniform float thinFaceDelta;
 uniform float bigEyeDelta;
 uniform float mouthResizeDelta;
 uniform float noseResizeDelta;
 ...
 // Signed radial scale around a feature center. delta > 0 magnifies the
 // feature (sampling pulled toward the center), delta < 0 shrinks it. Influence
 // falls off to zero at radius, so the rest of the face is untouched.
 vec2 scaleFeature(vec2 textureCoord, vec2 origin, float radius, float delta) {
     float r = distance(vec2(textureCoord.x, textureCoord.y / aspectRatio), vec2(origin.x, origin.y / aspectRatio)) / radius;
     if (r >= 1.0) {
         return textureCoord;
     }
     float falloff = 1.0 - r * r;
     float scale = 1.0 - delta * falloff;
     return origin + (textureCoord - origin) * scale;
 }

 vec2 mouthReshape(vec2 currentCoordinate) {
     vec2 leftCorner = vec2(facePoints[84 * 2], facePoints[84 * 2 + 1]);
     vec2 rightCorner = vec2(facePoints[90 * 2], facePoints[90 * 2 + 1]);
     vec2 topLip = vec2(facePoints[87 * 2], facePoints[87 * 2 + 1]);
     vec2 bottomLip = vec2(facePoints[93 * 2], facePoints[93 * 2 + 1]);
     vec2 center = (leftCorner + rightCorner + topLip + bottomLip) * 0.25;
     float width = distance(vec2(leftCorner.x, leftCorner.y / aspectRatio), vec2(rightCorner.x, rightCorner.y / aspectRatio));
     float radius = width * 0.85;
     return scaleFeature(currentCoordinate, center, radius, mouthResizeDelta);
 }

 vec2 noseReshape(vec2 currentCoordinate) {
     vec2 leftAla = vec2(facePoints[80 * 2], facePoints[80 * 2 + 1]);
     vec2 rightAla = vec2(facePoints[81 * 2], facePoints[81 * 2 + 1]);
     vec2 tip = vec2(facePoints[48 * 2], facePoints[48 * 2 + 1]);
     vec2 subnasale = vec2(facePoints[49 * 2], facePoints[49 * 2 + 1]);
     vec2 center = (leftAla + rightAla + tip + subnasale) * 0.25;
     float width = distance(vec2(leftAla.x, leftAla.y / aspectRatio), vec2(rightAla.x, rightAla.y / aspectRatio));
     float radius = width * 1.15;
     return scaleFeature(currentCoordinate, center, radius, noseResizeDelta);
 }
 ...
 void main()
 {
     ...
     if (hasFace == 1) {
         ...
         positionToUse = mouthReshape(positionToUse);
         positionToUse = noseReshape(positionToUse);
     }
...
bool FaceReshapeFilter::Init() {
  ...
  RegisterProperty("mouth_resize", 0,
                   "Mouth enlarge/shrink, range between -1 and 1.",
                   [this](float& val) { SetMouthResizeLevel(val); });

  RegisterProperty("nose_resize", 0,
                   "Nose/ala enlarge/shrink, range between -1 and 1.",
                   [this](float& val) { SetNoseResizeLevel(val); });
  ...
  this->mouth_resize_delta_ = 0.0;
  this->nose_resize_delta_ = 0.0;
  return true;
}
...
bool FaceReshapeFilter::DoRender(bool updateSinks) {
  ...
  filter_program_->SetUniformValue("mouthResizeDelta",
                                   this->mouth_resize_delta_);

  filter_program_->SetUniformValue("noseResizeDelta", this->nose_resize_delta_);
...
#pragma mark - mouth resize
void FaceReshapeFilter::SetMouthResizeLevel(float level) {
  mouth_resize_delta_ = level;
}

#pragma mark - nose resize
void FaceReshapeFilter::SetNoseResizeLevel(float level) {
  nose_resize_delta_ = level;
}

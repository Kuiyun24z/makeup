...
function drawLandmarks(values) {
  ...
  for (let i = 0; i < values.length; i += 2) {
    ...
    inputCtx.beginPath();
    inputCtx.arc(x, y, 2.4, 0, Math.PI * 2);
    inputCtx.fill();
  }
  inputCtx.restore();
}
...
function detectLandmarks() {
  ...
  drawLandmarks(landmarkValues);
  setStatus(`已生成 ${LANDMARK_POINT_COUNT} 个 GPUPixel 点。`);
  return true;
}
...
async function processImage(manual = true) {
  ...
  if (landmarker && landmarkValues.length !== LANDMARK_VALUE_COUNT) {
    setStatus("正在检测关键点...");

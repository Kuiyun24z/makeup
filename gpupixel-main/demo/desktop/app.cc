...
std::string input_landmarks_path_;
std::vector<float> external_landmarks_;
...
std::vector<float> LoadLandmarksFromTextFile(const std::string& path) {
  ...
  if (!file.is_open()) {
    std::cout << "[App] Landmarks file not found: " << path << std::endl;
    return {};
  }
  ...
  if (values.size() < 106 * 2) {
    std::cerr << "[App] Landmarks file has " << values.size()
              << " floats, expected at least " << 106 * 2 << std::endl;
    return {};
  }

  std::cout << "[App] Loaded " << values.size() / 2
            << " external face landmarks from: " << path << std::endl;
  return values;
}
...
void SetupFilterPipeline() {
  ...
  std::string image_path =
      input_image_path_.empty() ? GetExecutablePath() + "/demo.png"
                                : ResolveInputPath(input_image_path_);
  std::cout << "[App] Using image path: " << image_path << std::endl;
  source_image_ = SourceImage::Create(image_path);
  ...
  if (!input_landmarks_path_.empty()) {
    external_landmarks_ =
        LoadLandmarksFromTextFile(ResolveInputPath(input_landmarks_path_));
  } else {
    external_landmarks_ =
        LoadLandmarksFromTextFile(GetExecutablePath() + "/landmarks.txt");
  }
}
...
void RenderFrame() {
  ...
  std::vector<float> landmarks = external_landmarks_.empty()
                                     ? face_detector_->Detect(
...
#else
  if (!external_landmarks_.empty()) {
    lipstick_filter_->SetFaceLandmarks(external_landmarks_);
    blusher_filter_->SetFaceLandmarks(external_landmarks_);
    reshape_filter_->SetFaceLandmarks(external_landmarks_);
  }
#endif
...
int main(int argc, char** argv) {
  if (argc > 1) {
    input_image_path_ = argv[1];
  }
  if (argc > 2) {
    input_landmarks_path_ = argv[2];
  }

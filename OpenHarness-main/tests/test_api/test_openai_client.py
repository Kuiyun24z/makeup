...
@pytest.mark.asyncio
async def test_doubao_plhd_inline_tool_call_is_not_streamed_as_text():
    ...
    client._client = SimpleNamespace(
        chat=SimpleNamespace(completions=_InlineToolCompletions())
    )
    request = ApiMessageRequest(
    ...
    assert not [event for event in events if isinstance(event, ApiTextDeltaEvent)]
    complete = next(
    ...
    assert complete.message.tool_uses[0].name == "inspect_current_beauty_frame"
    assert complete.message.tool_uses[0].input == {"request_id": "vision-1"}
